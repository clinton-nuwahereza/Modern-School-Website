...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <?php 
    $prime_playschool_classes_meta_order = get_theme_mod('prime_playschool_classes_blog_meta_order', array('heading', 'author', 'featured-image', 'content', 'button'));
    
    foreach ($prime_playschool_classes_meta_order as $prime_playschool_classes_order) :
        if ('heading' === $prime_playschool_classes_order) :
        ...
        if ('author' === $prime_playschool_classes_order) :
            if ('post' === get_post_type() && $prime_playschool_classes_meta_setting) { ?>
        ...
        if ('featured-image' === $prime_playschool_classes_order) :
                ...
                <?php echo (!is_single()) ? '<a href="' . esc_url(get_the_permalink()) . '" class="post-thumbnail">' : '<div class="post-thumbnail">'; ?>
                    <?php if (has_post_thumbnail()) {
                        if (is_active_sidebar('right-sidebar')) {
                            the_post_thumbnail('prime-playschool-classes-with-sidebar', array('itemprop' => 'image'));
                        } else {
                            the_post_thumbnail('prime-playschool-classes-without-sidebar', array('itemprop' => 'image'));
                        }
                    } else { ?>
        ...
        if ('content' === $prime_playschool_classes_order) :
                ...
                <div class="entry-content" itemprop="text">
                    <?php if (is_single()) {
                        the_content(
                            sprintf(
                                wp_kses(
                                    __('Continue reading %s <span class="meta-nav">&rarr;</span>', 'prime-playschool-classes'),
        ...
        if ('button' === $prime_playschool_classes_order) :
            if (!is_single() && $prime_playschool_classes_read_more_setting) { ?>
