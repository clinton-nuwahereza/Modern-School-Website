...
function di_blog_post_sidebar_file_clbk() {
	// Display the added widgets otherwise display some defaults
	if( is_active_sidebar( 'sidebar-1' ) ) {
		dynamic_sidebar( 'sidebar-1' );
	} else {
		$args = array(
			'before_widget' => '<div class="widget widget_sidebar_main clearfix %s">', 
			'after_widget' => '</div>',
			'before_title' => '<h3 class="right-widget-title">',
			'after_title' => '</h3>'
		);

		$instance = array( 'title' => 'Search website' );
		the_widget( 'WP_Widget_Search', $instance, $args  );

		$instance = array( 'title' => 'Recent Posts' );
		the_widget( 'WP_Widget_Recent_Posts', $instance, $args  );

		$instance = array( 'title' => 'Categories' );
		the_widget( 'WP_Widget_Categories', $instance, $args  );

		$instance = array( 'title' => 'Tags' );
