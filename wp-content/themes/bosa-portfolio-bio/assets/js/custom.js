...
*/
function bosa_portfolio_bio_slickNavHeight (){
...
*/
function bosa_portfolio_bio_wpMenuAccordion( selector ){
...
function bosa_portfolio_bio_fixed_header(){
  $notificationHight = $( '.notification-bar' ).height();
  $logo_selector = document.getElementById( 'headerLogo' );
  var width = $( window ).width();

  if ( $logo_selector && BOSAPORTFOLIOBIO.fixed_nav && BOSAPORTFOLIOBIO.fixed_header_logo ) { 
    if ( $mastheadHeight < $(window).scrollTop()){
      if ( BOSAPORTFOLIOBIO.separate_logo == '' ){
        if ( BOSAPORTFOLIOBIO.the_custom_logo !== '' ){
          $logo_selector.src = BOSAPORTFOLIOBIO.the_custom_logo;
        }
      }else{
        $( '.site-header .site-branding img' ).css( 'display' , 'block' );
        if( !BOSAPORTFOLIOBIO.mobile_fixed_nav_off || width >= 782 ){
          $logo_selector.src = BOSAPORTFOLIOBIO.separate_logo;
        }
      }
    }else{
      if ( BOSAPORTFOLIOBIO.header_two_logo !== '' && ( BOSAPORTFOLIOBIO.is_front_page || BOSAPORTFOLIOBIO.overlay_post || BOSAPORTFOLIOBIO.overlay_page ) && BOSAPORTFOLIOBIO.is_header_two ){
         $logo_selector.src = BOSAPORTFOLIOBIO.header_two_logo;
      }else if ( BOSAPORTFOLIOBIO.the_custom_logo !== '' ) {
          $logo_selector.src = BOSAPORTFOLIOBIO.the_custom_logo;
      }else if ( BOSAPORTFOLIOBIO.separate_logo !== '' ){
        $( '.site-header .site-branding img' ).css( 'display' , 'none' );
      }
    }
  }
  if ( $mastheadHeight > $( window ).scrollTop() || $( window ).scrollTop() == 0 ) {
      if ( BOSAPORTFOLIOBIO.fixed_nav && $( '#masthead.site-header' ).hasClass( 'sticky-header' )){
          $( '#masthead.site-header' ).removeClass( 'sticky-header' );
          // Fixed header in admin bar
          if( BOSAPORTFOLIOBIO.is_admin_bar_showing && width >= 782 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
          if( BOSAPORTFOLIOBIO.is_admin_bar_showing && width <= 781 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
      }
  }else if ( BOSAPORTFOLIOBIO.fixed_nav && !$( '#masthead.site-header' ).hasClass( 'sticky-header' )){
    if( !BOSAPORTFOLIOBIO.mobile_fixed_nav_off || width >= 782 ){
      ...
      if( BOSAPORTFOLIOBIO.is_admin_bar_showing && width <= 600 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
      }
      if( BOSAPORTFOLIOBIO.mobile_fixed_nav_off && width <= 781 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
    }
  }
}
...
*/
function bosa_portfolio_bio_header_two_postion() {
  var width = $( window ).width();

  if ( BOSAPORTFOLIOBIO.is_header_two ) {
    if( BOSAPORTFOLIOBIO.is_admin_bar_showing && width >= 782 ){
...
jQuery( document ).ready( function() {
  bosa_portfolio_bio_slickNavHeight();
  bosa_portfolio_bio_wpMenuAccordion( '#offcanvas-menu' );
  bosa_portfolio_bio_header_two_postion();
  ...
  */
  $( '.header-image-slider' ).slick({
      ...
      fade: BOSAPORTFOLIOBIO.header_image_slider.fade,
      speed: parseInt( BOSAPORTFOLIOBIO.header_image_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSAPORTFOLIOBIO.header_image_slider.autoplay,
      autoplaySpeed: BOSAPORTFOLIOBIO.header_image_slider.autoplaySpeed,
      infinite: true,
  ...
  */
  $( '#primary-menu' ).slicknav({
      ...
      label: BOSAPORTFOLIOBIO.responsive_header_menu_text,
  ...
  */
  $( '.main-slider' ).slick({
      ...
      arrows: true,
      adaptiveHeight: false,
      fade: BOSAPORTFOLIOBIO.main_slider.fade,
      speed: parseInt( BOSAPORTFOLIOBIO.main_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSAPORTFOLIOBIO.main_slider.autoplay,
      autoplaySpeed: BOSAPORTFOLIOBIO.main_slider.autoplaySpeed,
      infinite: true,
  ...
  $( '.highlight-post-slider' ).slick({
      arrows: true,
      dots: true,
      slidesToShow: BOSAPORTFOLIOBIO.home_highlight_posts.slidesToShow,
      slidesToScroll: 1,
      adaptiveHeight: false,
      autoplay: BOSAPORTFOLIOBIO.home_highlight_posts.autoplay,
      autoplaySpeed: BOSAPORTFOLIOBIO.home_highlight_posts.autoplaySpeed,
      infinite: false,
      rows: 0,
  ...
  * Sticky sidebar
  */
  if( BOSAPORTFOLIOBIO.sticky_sidebar ){
...
jQuery( window ).on( 'resize', function(){
  bosa_portfolio_bio_slickNavHeight();
  bosa_portfolio_bio_fixed_header();
  bosa_portfolio_bio_header_two_postion();
});
...
jQuery( window ).on( 'load', function(){
  ...
  * Back to top
  */
  if( BOSAPORTFOLIOBIO.enable_scroll_top == true && $(window).scrollTop() > 200 ){
...
jQuery( window ).on( 'scroll', function(){
  ...
  * Back to top
  */
  if( BOSAPORTFOLIOBIO.disable_scroll_top == false && $(window).scrollTop() > 200 ){
    $( '#back-to-top' ).fadeIn( 200 );
  } else{
    $( '#back-to-top' ).fadeOut( 200 );
  }
