...
add_action('admin_notices', 'nursery_school_global_promo_notice');
function nursery_school_global_promo_notice() {
    // Only show if the theme exists in wp-content/themes/
    $theme_dir = WP_CONTENT_DIR . '/themes/nursery-school';
    if (!file_exists($theme_dir)) return;

    // Optional: only for admin users
    if (!current_user_can('manage_options')) return;

    $screenshot = content_url('/themes/nursery-school/screenshot.png');
    ?>
    <div class="notice nursery-school-notice">
        ...
        <div style="display:flex; align-items:center; gap:20px;margin-top: 15px;">
            <img src="<?php echo esc_url($screenshot); ?>" alt="Nursery School Theme" style="max-width:180px; height:auto; border:1px solid #ccc; border-radius:6px;">
            <div style="flex:1;">
                <strong style="font-size:16px;">Try The Nursery School Theme Pro!</strong><br>
                Unlock premium features, layouts, and priority support.
                <div style="margin-top:10px;">
                    <a href="https://www.logicaldemo.com/nursery-school/" target="_blank" class="button button-primary" style="background:#000; color:#fff;">View Demo</a>
                    <a href="https://www.logicalthemes.com/products/nursery-school-wordpress-template/" target="_blank" class="button" style="background:#0073aa; color:#fff;padding: 0 20px; border: 1px solid #ff0000; font-weight: 700;">Get Premium</a>
                </div>
            </div>
        </div>
    </div>
    <style>
        .nursery-school-notice {
